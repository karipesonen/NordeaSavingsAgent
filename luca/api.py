...
GET  /daily-recap                              →  serve latest stored daily brief
POST /run-daily-recap                          →  trigger the daily recap agent now
GET  /health
"""
...
async def daily_recap_endpoint():
    """Return the most recently stored daily brief (generated by the agent at 18:00)."""
    from agents.daily_recap_agent import load_latest_brief, run_daily_recap
    brief = load_latest_brief()
    if brief is None:
        # No stored brief yet — generate one on demand
        try:
            brief = run_daily_recap()
        except Exception as e:
            raise HTTPException(status_code=500, detail=str(e))
    return brief
...
async def run_daily_recap_now():
    """Manually trigger the daily recap agent (also called by the scheduler)."""
...
# Runs every minute. Runs in a daemon thread so it shuts down with the server.

def _daily_recap_scheduler():
    while True:
        time.sleep(60)
        try:
            from agents.daily_recap_agent import run_daily_recap
            run_daily_recap()
            print(f"[daily recap] completed at {datetime.datetime.now().isoformat()}")
        except Exception as exc:
            print(f"[daily recap] error: {exc}")

threading.Thread(target=_daily_recap_scheduler, daemon=True, name="daily-recap").start()

...
- Mobile showcase transcript now uses two longer static loops instead of three: Emma demonstrates short-term trip planning, safety-first education, goal planning, approval, and what changes if the horizon extends; Sofia demonstrates detailed spending review, save/future-funds bridge, starter plan, approval, funds-before-stocks education, curated resource, and a first-fund-step explanation for a beginner. The showcase remains static JSON in `nora-mobile/public/data/showcase-transcript.json`, not live LLM generation.
- Mobile showcase and live mobile prompt now use soft guiding language: Nora proposes a practical next step and asks small choice questions only when useful, such as building a starter plan, choosing confirmation, or taking the 30-second explanation. The goal is guidance over interrogation.
- Mobile showcase playback now pauses whenever the user opens a tab/card detail or drawer, and it only resumes from the chat view if the chat is actually at the bottom. This prevents the auto-cycle from advancing to Sofia while the presenter is reading a Learn/resource card or has the chat scrolled up. Follow-up fix: the initial showcase load must not treat a missing/not-yet-mounted chat scroller as "scrolled up", otherwise showcase starts paused until the sidebar is opened/closed.
...
- ETF discovery prompts now bypass raw Luca investment-agent prose as well. Prompts such as `Show me ETFs` return a structured `etf_overview` card with beginner-friendly examples and safety wording. The static showcase does not use Sofia's portfolio/ETF holdings, but the live demo can still explain ETF examples without breaking the first-time-investor scenario.
- Personal ETF prompts are now separate from generic ETF discovery. Phrases such as `show my ETFs` route to Sofia's ETF holdings from the Luca portfolio summary and render a filtered `portfolio_summary` card titled `ETF holdings`; generic phrases like `show me ETFs` still show educational ETF examples.
...
- Changed the detailed Sofia expense review trust note from implementation wording (`Luca CSV spending data`) to customer-facing wording.
- Follow-up display polish: portfolio/ETF cards now use demo display date `2026-06-01` (shown as `1 Jun 2026` in mobile UI) instead of exposing the CSV market-data date. Detailed expense review cards now say `Last month` / `last month's spending picture` instead of exposing the underlying May 2025 fixture year.
- Added a conservative mobile backend paragraph formatter for Nora replies. Long single-paragraph messages can now be split into two short paragraphs at natural transition points such as `One possible path:`, `I added...`, or `Want me to...`; short replies, confirmations, banking confirmation cards, and errors are left unchanged.
- Replaced feature-like first-turn chips with three soft conversation entrances under Nora's opener: `I want to save more`, `I'm curious about investing`, and `I'm planning something`. These are starter-only chips, not a sidebar/menu; after the first user action, normal dynamic suggested replies take over.
- Added a deterministic follow-up guard for the `I'm planning something` starter path. Short replies like `A trip`, `purchase`, or `house deposit` now get a direct Nora clarifying question instead of relying on the LLM JSON orchestrator, preventing the generic "lost my train of thought" fallback on tiny fragments.

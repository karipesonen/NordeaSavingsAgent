...
function App() {
  ...
  React.useEffect(() => {
    ...
    fetch('/api/demo-profiles')
      ...
      .then(data => {
        ...
        if (!tweaks.profileId && data.defaultProfileId) {
          setTweak('profileId', data.defaultProfileId);
        }
      })
      .catch(err => {
        console.warn('Could not load demo profiles', err);
  ...
  const selectedProfile = tweaks.demoMode === 'test_profile'
    ? (profiles.find(p => p.userId === tweaks.profileId) || profiles[0] || SCRIPTED_PROFILE)
    : SCRIPTED_PROFILE;
  const sessionKey = [
    tweaks.demoMode || 'scripted_emma',
    tweaks.profileId || 'none',
    selectedProfile.userId || 'profile-loading',
    tweaks.firstReplyMode || 'hardcoded',
  ].join(':');
...
function NoraTweaks({ tweaks, setTweak, profiles, profilesReady }) {
  const setDemoMode = (mode) => {
    ...
    if (mode === 'test_profile' && !profiles.some(p => p.userId === tweaks.profileId) && profiles[0]) {
      edits.profileId = profiles[0].userId;
    }
    setTweak(edits);
  };
  ...
  return (
    <TweaksPanel title="Tweaks · Nora demo">
      <TweakSection label="Demo data">
        <TweakRadio
          label="Data source"
          value={tweaks.demoMode || 'scripted_emma'}
        ...
        {tweaks.demoMode === 'test_profile' && (
          <div className="twk-row">
            <div className="twk-lbl"><span>Profile</span></div>
            <select
              className="twk-field"
              value={tweaks.profileId || profiles[0]?.userId || ''}
              disabled={!profilesReady || !profiles.length}
        ...
        <TweakRadio
          label="First reply"
          value={tweaks.firstReplyMode || 'hardcoded'}
          onChange={(v) => setTweak('firstReplyMode', v)}
          options={[
            { value: 'hardcoded',       label: 'Hardcoded' },
            { value: 'agent_generated', label: 'Agent' },
          ]}
        />
      </TweakSection>

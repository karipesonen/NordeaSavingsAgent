...
// ── Drawer ─────────────────────────────────────────────────────────────────
function Drawer({ open, onClose, activeTab, onPick, counts }) {
  const items = [
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontSize: 17, fontWeight: 700, marginTop: 2 }}>Emma · with Nora</div>
          </div>
    ...
    <InChatCard eyebrow={`Expense review · ${createdAt ? formatRelative(createdAt) : 'last 90 days'}`} vibe="balanced">
      <div style={{ padding: '16px 20px 18px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, marginBottom: 18 }}>
          <MoneyDisplay value={data.weeklyRoom || 0} size={36} color="var(--fg-1)" />
          <span style={{ fontSize: 13, color: 'var(--fg-3)', fontWeight: 500 }}>/ week, roughly</span>
        </div>
        {items.map((row, i) => (
          <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 0', borderBottom: i < items.length - 1 ? '1px solid var(--border-1)' : 'none' }}>
            ...
            <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--fg-1)', fontVariantNumeric: 'tabular-nums lining-nums' }}>−{euroN(row.weeklyAmount)}/wk</div>
          </div>
        ))}
      </div>
    </InChatCard>
    ...
    <div style={{ width: 'calc(100% - 38px)', marginLeft: 38 }}>
      ...
      onMouseLeave={(e) => { e.currentTarget.style.borderColor = 'var(--border-1)'; e.currentTarget.style.background = '#fff'; }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--fg-1)', lineHeight: 1.35 }}>
            €{data.weeklyRoom || 0}/wk to redirect · {(data.items || []).length} categories
          </div>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 12, fontWeight: 600, color: NORA_BLUE, flexShrink: 0 }}>
          Open in Spending
